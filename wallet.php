    ...
    <div class="studio-container">
        ...
        <header class="dash-header" style="border:none; margin-bottom: 20px;">
            ...
            <div class="header-right">
                <div class="avatar" style="background:var(--accent-color); color:white; display:flex; justify-content:center; align-items:center; font-weight:bold; cursor:pointer; overflow:hidden; width:36px; height:36px;" onclick="location.href='profile.php'">
        ...
        <?php if ($msg): ?>
            <div class="<?php echo strpos(strtolower($msg), 'success') !== false || strpos(strtolower($msg), 'requested') !== false ? 'success-message' : 'error-message'; ?>" style="margin-bottom: 25px;">
        ...
        <div class="wallet-grid" style="display: grid; grid-template-columns: 1fr 1.5fr; gap: 40px;">
            ...
            <div style="display: flex; flex-direction: column; gap: 20px;">
                <div class="revenue-card" style="flex-direction: column; align-items: flex-start; gap: 20px; padding: 40px; margin-bottom: 0;">
                    <div class="revenue-content">
                        <span class="revenue-label" style="background: rgba(138, 43, 226, 0.2); color: #a45eff;">Current Balance</span>
                        <h1 style="color: white; margin: 10px 0;">₹<?php echo number_format($_SESSION['wallet_balance'], 2); ?></h1>
                        <p style="color: #444; font-size: 13px;">Available funds in your RenderShelf wallet</p>
                    </div>
                    <button onclick="document.getElementById('withdraw-modal').classList.add('active')" class="btn-primary" style="width: 100%; border-radius: 12px; font-size: 15px; margin: 0;">
                        <ion-icon name="arrow-down-circle" style="font-size: 20px;"></ion-icon> Withdraw Funds
                    </button>
                    <div style="font-size: 11px; color: #333; text-align: center; width: 100%; display: flex; align-items: center; justify-content: center; gap: 5px;">
                ...
                <div style="max-height: 550px; overflow-y: auto; padding-right: 15px;" class="custom-scrollbar">
                    <?php if ($transactions->num_rows > 0): ?>
                        <?php while($row = $transactions->fetch_assoc()): ?>
                            ...
                            ?>
                            <div class="transaction-item" style="background: var(--card-bg); padding: 16px; border-radius: 16px; margin-bottom: 12px; display: flex; align-items: center; justify-content: space-between; border: 1px solid rgba(255,255,255,0.03); transition: transform 0.2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='translateY(0)'">
                                <div style="display: flex; align-items: center; gap: 15px;">
                                    <div style="width: 44px; height: 44px; border-radius: 12px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; font-size: 20px; color: <?php echo $is_positive ? '#38ef7d' : '#ff4444'; ?>;">
                                        <ion-icon name="<?php echo $icon; ?>"></ion-icon>
                                    </div>
                                    <div>
                                        <div style="font-size: 14px; font-weight: 700; color: white;"><?php echo htmlspecialchars($row['description'] ?: $row['type']); ?></div>
                                        <div style="font-size: 11px; color: #555; font-weight: 600; margin-top: 2px;"><?php echo date('M d, Y • h:i A', strtotime($row['created_at'])); ?></div>
                                    </div>
                                </div>
                                <div style="font-size: 15px; font-weight: 800; color: <?php echo $is_positive ? '#38ef7d' : 'white'; ?>;">
                                    <?php echo $is_positive ? '+' : ''; ?>₹<?php echo number_format($row['amount'], 2); ?>
                                </div>
                            </div>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <div style="text-align: center; padding: 80px 20px; background: rgba(255,255,255,0.02); border-radius: 30px; border: 1px dashed rgba(255,255,255,0.1);">
                            <ion-icon name="receipt-outline" style="font-size: 48px; margin-bottom: 20px; color: #222;"></ion-icon>
        ...
        <div class="withdrawal-modal">
            ...
            <form method="POST">
                <div class="amount-input-container">
                    ...
                    <input type="number" id="withdraw-amount" name="amount" class="amount-input" placeholder="0.00" required min="1" max="<?php echo $_SESSION['wallet_balance']; ?>" step="0.01">
                    <div class="max-btn" onclick="document.getElementById('withdraw-amount').value = '<?php echo $_SESSION['wallet_balance']; ?>'">MAX</div>
                </div>
                
                <div style="display:flex; justify-content:space-between; font-size:12px; color:var(--text-secondary); margin-bottom:20px; padding:0 5px;">
                    <span>Available Balance</span>
                    <span>₹<?php echo number_format($_SESSION['wallet_balance'], 2); ?></span>
                </div>

                <button type="submit" name="withdraw" class="withdraw-btn">Request Withdrawal</button>
                <div onclick="document.getElementById('withdraw-modal').classList.remove('active')" class="cancel-link">Cancel</div>
            </form>
        </div>
    </div>
