...
$all_assets = $conn->query("SELECT id, title FROM assets WHERE is_approved=1 ORDER BY title ASC");
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <main class="main-content">
        <div class="admin-container">
            <header class="admin-header">
                ...
                <button class="btn-primary" onclick="showModal()" style="width:auto; padding:10px 25px;">+ Add
                    Tutorial</button>
            </header>
                ...
                <?php while ($t = $tutorials->fetch_assoc()): ?>
                    <div class="tut-admin-card">
                        ...
                        <div style="padding: 15px;">
                            ...
                            <p style="font-size:12px; color:#666; margin: 5px 0;">By
                                <?php echo htmlspecialchars($t['author_name']); ?> • <?php echo $t['duration']; ?></p>
                            <p
                                style="font-size:11px; color:var(--accent-color); overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                                <?php echo htmlspecialchars($t['video_url']); ?></p>
                        </div>
                        <div class="tut-actions">
                            <button class="view-btn" onclick='editTut(<?php echo json_encode($t); ?>)'
                                style="flex:1;"><ion-icon name="create-outline"></ion-icon> Edit</button>
                            <a href="?delete=<?php echo $t['id']; ?>" class="view-btn"
                                style="flex:1; background:rgba(255,68,68,0.1); color:#ff4444;"
                                onclick="return confirm('Delete this tutorial?')"><ion-icon name="trash-outline"></ion-icon>
                                Delete</a>
                        </div>
                    </div>
                <?php endwhile; ?>
        ...
        <div class="modal-card" style="max-width: 500px; text-align: left;">
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group" style="margin:15px 0;">
                    ...
                    <input type="text" name="title" id="tut-title" required class="input-field"
                        style="background:#0a0a0a; margin-top:5px;">
                </div>
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:15px;">
                    <div class="form-group">
                        <label style="font-size:12px; color:#888;">Author Name</label>
                        <input type="text" name="author_name" id="tut-author" required class="input-field"
                            style="background:#0a0a0a; margin-top:5px;">
                    </div>
                    <div class="form-group">
                        <label style="font-size:12px; color:#888;">Duration (e.g. 15:30)</label>
                        <input type="text" name="duration" id="tut-duration" class="input-field"
                            style="background:#0a0a0a; margin-top:5px;">
                    </div>
                </div>
                <div class="form-group" style="margin:15px 0;">
                    <label style="font-size:12px; color:#888;">YouTube / Video URL</label>
                    <input type="url" name="video_url" id="tut-url" required class="input-field"
                        style="background:#0a0a0a; margin-top:5px;">
                </div>
                <div class="form-group" style="margin:15px 0; display: grid; grid-template-columns: 1fr 1fr; gap:15px;">
                    <div>
                        ...
                        <select name="category_id" id="tut-cat" class="input-field"
                            style="background:#0a0a0a; margin-top:5px;">
                            <?php
                    ...
                    <div>
                        ...
                        <select name="related_asset_id" id="tut-asset" class="input-field"
                            style="background:#0a0a0a; margin-top:5px;">
                            ...
                            while ($as = $all_assets->fetch_assoc()):
                                ?>
                                <option value="<?php echo $as['id']; ?>"><?php echo htmlspecialchars($as['title']); ?>
                                </option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                ...
                <div class="form-group" style="margin:15px 0;">
                    ...
                    <input type="file" name="thumb_img" class="input-field" style="background:#0a0a0a; margin-top:5px;"
                        accept="image/*">
                </div>
