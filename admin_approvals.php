...
if (isset($_GET['approve'])) {
    ...
    $conn->query("UPDATE assets SET is_approved=1 WHERE id=$id");
...
if (isset($_GET['reject'])) {
    ...
    $conn->query("UPDATE assets SET is_approved=2 WHERE id=$id"); // Assuming 2 is rejected
    // OR if there is no rejected status (since it's TINYINT 0/1), maybe delete? 
    // But typically 0=pending, 1=approved. If we need rejected, we might need value 2 or -1.
    // The previous schema check showed: is_approved TINYINT DEFAULT 0.
    // Let's stick to is_approved=1 for approved. 
    // For rejected, if we just set it back to 0 or delete, it's ambiguous. 
    // Ideally, we should have an ENUM or a separate status column, but for now let's assume 2=rejected or handle removal.
    // Re-reading confirm msg: `admin_delete_asset.php` exists. rejection might just be deleting?
    // The original code tried `status='rejected'`.
    // Let's check update log. `is_approved` is TINYINT. 0=pending/unapproved, 1=approved.
    // If I set is_approved=2 it might work if TINYINT(1) isn't strictly boolean in mysql (it's usually just TINYINT).
    header("Location: admin_approvals.php?msg=Asset Rejected");
    exit;
}

$result = $conn->query("SELECT a.*, u.username FROM assets a JOIN users u ON a.creator_id = u.id WHERE a.is_approved=0");
?>
<!DOCTYPE html>
<html lang="en">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div style="display:flex; gap:10px; align-items:center; justify-content: flex-end;">
                                        <a href="?approve=<?php echo $row['id']; ?>" class="btn-primary"
                                            style="padding:8px 16px; font-size:12px; background:#38ef7d; color:black; text-decoration:none; border-radius:8px; font-weight:bold; margin-top: 0; width: auto; box-shadow: none;">Approve</a>
                ...
                <?php if ($result->num_rows == 0): ?>
                    <div style="text-align:center; padding:80px 0; color:#444;">
                        <ion-icon name="checkmark-done-circle-outline"
                            style="font-size:64px; margin-bottom: 15px; color: #333;"></ion-icon>
